...
const REF_JSON = joinpath(@__DIR__, "..", "data", "reference_trajectory", "pusher_ref_traj.json")
const OUT_DIR = joinpath(@__DIR__, "..", "data")
const OUT_PNG = joinpath(OUT_DIR, "planar_push_ref_contact_force_smoothed.png")
...
function main()
    ...
    nc = length(gammas[1])
    nc >= 1 || error("expected at least 1 contact force, got nc=$(nc)")

    g_raw = [Float64[g[i] for g in gammas] for i in 1:nc]
    g_dense = [linear_resample(t, g_raw[i], td) for i in 1:nc]
    g_smooth = [smooth_gaussian(g_dense[i], kernel) for i in 1:nc]
    g_sum = [sum(g_smooth[j][i] for j in 1:nc) for i in eachindex(td)]
    ...
    p = plot(
        ...
        xlabel="time [s]",
        ylabel="normal force",
        title="Planar Push Contact Normal Forces (Smoothed from Reference)",
        linewidth=2.5,
        alpha=0.95,
    )
    for i in 2:nc
        plot!(p, td, g_smooth[i], label="gamma$(i)", linewidth=2.5, alpha=0.95)
    end
    ...
    println("num contact forces (nc): ", nc)
end
