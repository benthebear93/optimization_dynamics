...
RoboDojo.nominal_configuration(model::LinePlanarPush) = [0.0; -0.1 - 1.0e-8; 0.025;  -0.1 - 1.0e-8; -0.025]
...
function RoboDojo.indices_optimization(model::LinePlanarPush)
    ...
    socri = Vector{Vector{Int}}([
        collect((nq + 20) .+ (1:3)),  # nq+17:nq+19
        collect((nq + 23) .+ (1:3)),  # nq+20:nq+22
        collect((nq + 26) .+ (1:3)),  # nq+23:nq+25
        collect((nq + 29) .+ (1:3)),  # nq+26:nq+28
        collect((nq + 32) .+ (1:2)),  # nq+29:nq+30
        collect((nq + 34) .+ (1:2)),  # nq+29:nq+30
    ])
    # bil: Python (nq+15):(nq+29) -> Julia (nq+16):(nq+30)
    bil = collect((nq + 18) .+ (1:18))
    # println("nz ", nz)
    # println("n_delta ", nz)
    # println("ortz ", [ort1, ort2])
    # println("ort_delta ", [ort1, ort2])
    # println("socz ", socz)
    # println("socΔ ", socΔ)
    # println("equr ", equr)
    # println("ortr ", ortr)
...
function RoboDojo.indices_θ(model::LinePlanarPush; nf=0) 
    nq = model.nq 
    nu = model.nu 
    nw = model.nw 
...
RoboDojo.num_var(model::LinePlanarPush) = 41 # model.nq + 2 + 2 + 6 + 10 + 6 + 10
